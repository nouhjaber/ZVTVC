...
class FlowMatchingUNet(nn.Module):
    ...
    Conditioning:
        - Time: Added to each ResBlock
        - Timbre: Via AdaLN in ResBlocks (global)
        - Content+Prosody: Via cross-attention (frame-level)
    """
    def __init__(
        self,
        mel_channels: int = 80,
...
class FlowMatchingGenerator(nn.Module):
    ...
    Wraps UNet and provides training/inference interfaces.

    For training:
        - Samples time t ~ Uniform(0, 1)
        - Computes noisy sample x_t = (1-t)*x0 + t*x1
        - Predicts velocity v = x1 - x0
        - Loss: MSE(v_pred, v_target)

    For inference:
        - Start from noise x0 ~ N(0, I)
        - Integrate ODE: dx/dt = v(x, t, conditions)
        - Return final sample x1 (mel-spectrogram)
    """
